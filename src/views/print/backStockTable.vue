  ...
  <div id='goStockTable'
    ...
    <div class="printTable crosswise">
      ...
      <div class="print_body">
        <div class="print_row">
          <div class="row_item center">回库时间</div>
          <div class="row_item left w180">物料名称</div>
          <!-- <div class="row_item left">出库仓库</div> -->
          <div class="row_item left">颜色属性</div>
          <div class="row_item left">批号/缸号</div>
          <div class="row_item left">色号</div>
          <div class="row_item right">回库数量</div>
          <div class="row_item right">回库件数</div>
          <!-- <div class="row_item left">出库款号</div> -->
          <div class="row_item left">回库单位</div>
        </div>
        <div class="print_row"
          v-for="(item,index) in logInfo"
          :key="index">
          <div class="row_item center">{{$getTime(item.complete_time)}}</div>
          ...
          <!-- <div class="row_item left">{{item.desc}}</div> -->
          <div class="row_item left">{{item.client_name}}</div>
        </div>
        <div class="print_row"
...
export default {
  data () {
    return {
      title: window.sessionStorage.getItem('company_name') + '物料回库单',
      ...
      yarnOrder.detail({
        ...
        if (res.data.status !== false) {
          ...
          if (logIdArr) {
            ...
            this.logInfo = data.stock_log.filter(itemF => itemF.action_type === 3)
          }
          setTimeout(() => {
            window.print()
          }, 1000)
  ...
  created () {
    ...
    this.title = (printInfo && JSON.parse(printInfo) && (this.$route.params.type === '2' ? JSON.parse(printInfo).material_back : JSON.parse(printInfo).raw_back)) || this.title
